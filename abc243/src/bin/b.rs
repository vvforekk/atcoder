...
fn main() {
    ...
    let ans1 = (0..n).filter(|&x| a[x] == b[x]).count();

    let ma = a.into_iter().collect::<BTreeSet<usize>>();
    let mb = b.into_iter().collect::<BTreeSet<usize>>();
    let ans2 = ma.intersection(&mb).count() - ans1;
    vis!(ans1; ans2);
}
